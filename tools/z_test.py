...
# -*- coding: utf-8 -*-
"""
z_test.py  -  Test truc Z (len xuong) qua PLC Modbus TCP truc tiep.

Go VI TRI Z (mm), bam CHAY -> Z chay toi do. Giu X tai cho (chi Z di).
Chi can Python chuan. LAN cam thang laptop <-> PLC.

    python tools/z_test.py
    python tools/z_test.py --host 192.168.0.10

!!! Z LA TRUC DUNG: mat dien la dau Z co the ROI. Ke do ben duoi khi test.
...
P_VEL_Z = 3
DEFAULT_FACTOR = 1.0     # mm mode -> he so 1 (go mm thang)
DEFAULT_VEL = 30.0       # mm/s (Z stall ~80, de 30 cho an toan)
...
class Plc:
    ...
    # MOVE_XZ: giu X tai cho (doc X hien tai) de chi Z di
    def move_z(s, z):
        x_now = s.status()["x"]
        return s._wait(s._send(C_MOVE_XZ, x=x_now, z=z), 120)
    ...
    def set_vel(s, v): return s._wait(s._send(C_SET_PARAM, sel=P_VEL_Z, x=v), 10)
...
class App:
    def __init__(s, root, host, port):
        ...
        root.title("Test truc Z"); root.configure(bg=BG)
        root.geometry("440x500"); root.minsize(420, 480)
        ...
        # canh bao truc dung
        tk.Label(root, text="⚠ Z la truc DUNG - ke do ben duoi keo roi",
                 bg=BG, fg=REDD, font=("Segoe UI", 9, "bold")).pack(pady=(6, 0))

        card = tk.Frame(root, bg=CARD); card.pack(fill="both", expand=True, padx=14, pady=10)

        tk.Label(card, text="TRUC Z  (len xuong)", bg=CARD, fg=SUB,
                 font=("Segoe UI", 10, "bold")).pack(pady=(12, 2))
        tk.Label(card, text="Vi tri Z muon toi (mm)", bg=CARD, fg=INK,
                 font=("Segoe UI", 13)).pack(pady=(4, 4))

        s.pos = tk.StringVar(value="50")
        ...
        act = tk.Frame(card, bg=CARD); act.pack(pady=(10, 4))
        tk.Label(act, text="Z hien tai:", bg=CARD, fg=SUB, font=("Segoe UI", 12)).pack(side="left")
        s.act = tk.StringVar(value="-")
        tk.Label(act, textvariable=s.act, bg=CARD, fg=GREEND,
                 font=("Consolas", 20, "bold")).pack(side="left", padx=8)

        row2 = tk.Frame(card, bg=CARD); row2.pack(pady=6)
        ...
        tk.Label(r, text="he so:", bg="#f3f6f9", fg=INK, font=("Segoe UI", 9)).pack(side="left")
        s.factor = tk.StringVar(value=f"{DEFAULT_FACTOR:g}")
        tk.Entry(r, textvariable=s.factor, width=6, relief="solid", bd=1).pack(side="left", padx=(4, 12))
        tk.Label(r, text="toc(mm/s):", bg="#f3f6f9", fg=INK, font=("Segoe UI", 9)).pack(side="left")
    ...
    def _factor(s):
        f = float(s.factor.get()); return f if f else 1.0
    ...
    def _bg(s, val):
        ...
        def w():
            try:
                ...
                s.msg.set(f"chay Z toi {val:g} (gui {target:.0f})...")
                r = s.plc.move_z(target)
                ok = {0:"?",1:"dang chay",2:"XONG",3:"bi dung",4:"LOI",5:"tu choi"}
                s.msg.set(f"Z toi {val:g}: {ok.get(r['result'],'?')}"
    ...
    def _upd(s, online, st):
        ...
        else: s.banner.config(text="● CHUA HOME  (bam CHAY se tu home)", bg=ORANGE)
        try: s.act.set(f"{st['z'] / s._factor():.1f}")
